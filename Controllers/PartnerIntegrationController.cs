...
namespace API_HUNT.Controllers
{
    public class PartnerIntegrationController : Controller
    {
        ...
        private readonly IPartnerRepository _legacyPartnerRepo;
        private readonly IActivityLogRepository _activityLog;
        private readonly string uploadFolderPath;
        public PartnerIntegrationController(IPartnerOnboardingNewRepository partnerRepo, IPartnerRepository legacyPartnerRepo, IActivityLogRepository activityLog)
        {
            _partnerRepo = partnerRepo;
            _legacyPartnerRepo = legacyPartnerRepo;
        ...
        public IActionResult ListofPartner(string Status)
        {
            if (HttpContext.Session.GetString("Role").ToString() == "USER")
            {
                ...
                partnerOnboarding.lstPartnerIntegrationDetail = _partnerRepo.ListofIntegration();
            ...
            {
                return RedirectToAction("ListofPartner", "PartnerApproval");
            }
        }
        public IActionResult AddPartnerIntegration()
        {
            PartnerIntegrationNew partnerOnboarding = new PartnerIntegrationNew();
            var PartnerNameList = (from Partner in _partnerRepo.GetPartnerNameList()
        ...
        public ActionResult GetPartnerDetails(string partnerID)
        {
            try
            {
                var partnerDetails = _partnerRepo.GetPartnerDetailIntegration(partnerID);
        ...
        {
            var objPartnerCaseApprovalMetrix = _partnerRepo.PartnerCaseApprovalMetrix(PartnerRiskClassification, APIRiskClassification, ApproverType, ApproverLevel);
        ...
        public JsonResult GetlstApiDetail(string prefix)
        {
            ...
            partner = _partnerRepo.GetlstApiDetail();
        ...
        {
            ApiDetailNew apiDetail = new ApiDetailNew();
            try
            {
                apiDetail = _partnerRepo.GetApiDetail(APIName);
            }
            catch (Exception ex)
            {
                ex.ToString();
        ...
        public IActionResult SavePartnerIntegration(string jsonPartnerOnboarding)
        {
            ...
            var fileData = _partnerRepo.GetNewCaseID();
            ...
            data = _partnerRepo.AddPartnerIntegration(data);
        ...
        {
            var partnerApproval = _partnerRepo.GetIntegrationDetail(id);
            ...
            partnerApproval.lstPOFeedbackHistory = _partnerRepo.GetFeedbackDetail(id);
        ...
        public IActionResult DisplayFile(string fileName)
        {
            ...
            foreach (string extension in supportedExtensions)
            {
                ...
                {
                    byte[] fileBytes = System.IO.File.ReadAllBytes(filePath);
                    if (extension == ".docx" || extension == ".xlsx" || extension == ".xls" || extension == ".zip" || extension == ".7z" || extension == ".doc")
                    {
                        return File(fileBytes, "application/octet-stream", fileName + extension);
                    }
                    else
        ...
        public IActionResult SaveEditPartnerIntegration(string jsonPartnerOnboarding)
        {
            try
            {
                ...
                var partnerEdit = _partnerRepo.EditPartnerIntegration(data);
                ...
                _activityLog.LogActivity(HttpContext.Session.GetString("EmpId") ?? string.Empty, "Partner Integration", "API HUNT", 1, "Partner Integration Add", "Partner Integration Add - " + HttpContext.Session.GetString("EmpId").ToString());
        ...
        public ActionResult DeletePartneronBoarding(string id)
        {
            try
            {
                bool isDeleted = _legacyPartnerRepo.DeletePartner(id);
                if (isDeleted)
                {
                ...
                _activityLog.LogActivity(HttpContext.Session.GetString("EmpId") ?? string.Empty, "Partner Integration", "API HUNT", 1, "Partner Integration Delete", "Partner Integration Delete - " + HttpContext.Session.GetString("EmpId").ToString());
